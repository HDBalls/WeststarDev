...
class Product(models.Model):
    ...
    target_price1 = fields.Monetary(string='Target Sales Price', store=True, compute='_compute_netprice', help='Target Sales Price for Weststar')
...
#        products = self.env['product.template'].search([('type', '=', 'product')])
    ...
    def _cal_update_gross_price_from_target_price(self):
        products = self.env['product.template'].search([('type', '=', 'product'), ('list_price', '<=', 1)])
        for product in products:
